...
if nargin < 3; VERBOSE = 3; end;
params.state_space_dimension = 2; % either 3 or 2
params.Sigma_Q = diag([10000 10000]); % measurement noise covariance matrix
params.M = 1000;
params.motion_type = 1; %0=fixed, 1=linear, 2=circular
...
switch params.state_space_dimension
    case 2
        params.Sigma_R = diag([10 10]); % process noise covariance matrix
       % if params.omega_0; error('2D state space can not use omega_0'); end;
    case 3
        
	params.omega_0 = 2*pi/688;
         params.Sigma_R = diag([2 2 0.001]);
end
dt = 1;
%%
